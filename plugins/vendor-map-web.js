...
function appJs() {
  ...
  const state = { data:null, selectedId:null, scale:1, x:0, y:0, imgW:0, imgH:0, mapSize:null, ocean:0, timer:null, expandedCheapest:{}, popoverScrollTop:0, hiddenItems:new Set(), vendorListVisible:false, annotations:{markers:[],strokes:[]}, drawMode:false, currentStroke:null };
  const els = {
    ...
    layer: document.getElementById('markerLayer'), drawCanvas: null, empty: document.getElementById('emptyMap'), details: document.getElementById('details'), detailsBody: document.getElementById('detailsBody'), toast: document.getElementById('toast'), homeX: document.getElementById('homeX'), homeY: document.getElementById('homeY'), homeRadius: document.getElementById('homeRadius'), homeStatus: document.getElementById('homeStatus'), refreshSeconds: document.getElementById('refreshSeconds'), hiddenItems: document.getElementById('hiddenItems'), toggleVendorList: document.getElementById('toggleVendorList')
  };
  ...
  function bindControls(){
    ...
    let drawing=false;
    els.map.addEventListener('pointerdown', (e)=>{ if(!state.drawMode) return; drawing=true; const pt={x:(e.offsetX-state.x)/state.scale,y:(e.offsetY-state.y)/state.scale}; state.currentStroke=[pt]; });
    els.map.addEventListener('pointermove', (e)=>{ if(!state.drawMode||!drawing) return; state.currentStroke.push({x:(e.offsetX-state.x)/state.scale,y:(e.offsetY-state.y)/state.scale}); renderAnnotations(); const ctx=els.drawCanvas.getContext('2d'); const st=state.currentStroke; if(st.length>1){ ctx.beginPath(); ctx.moveTo(st[st.length-2].x,st[st.length-2].y); ctx.lineTo(st[st.length-1].x,st[st.length-1].y); ctx.strokeStyle='#fbbf24'; ctx.lineWidth=4; ctx.stroke(); } });
    els.map.addEventListener('pointerup', async ()=>{ if(!state.drawMode||!drawing) return; drawing=false; if(state.currentStroke&&state.currentStroke.length>1){ state.annotations.strokes.push(state.currentStroke); await saveAnnotations(); } state.currentStroke=null; render(); });
  }
  ...
  function renderAnnotations(){
    ...
    cvs.style.transform = 'translate(' + state.x + 'px,' + state.y + 'px) scale(' + state.scale + ')';
    const ctx = cvs.getContext('2d'); ctx.clearRect(0,0,cvs.width,cvs.height); ctx.lineWidth = 4; ctx.strokeStyle = '#fbbf24'; ctx.lineCap='round';
    (state.annotations.strokes||[]).forEach(st=>{ if(!Array.isArray(st)||st.length<2)return; ctx.beginPath(); ctx.moveTo(st[0].x, st[0].y); st.forEach(pt=>ctx.lineTo(pt.x, pt.y)); ctx.stroke(); });
