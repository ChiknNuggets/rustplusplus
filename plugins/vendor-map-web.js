...
const itemIconState = { path: Path.join(__dirname, '..', 'src', 'staticFiles', 'item-icons-embedded.js'), mtimeMs: 0, icons: {} };
const FACEPUNCH_ITEM_ICON_BASE_URL = 'https://files.facepunch.com/rust/item/';
...
function getItem(client, itemId) {
  ...
  try {
    if (client.items && itemId != null) {
      ...
      icon = getEmbeddedItemIcon(itemId, shortName) || client.items.getImage?.(itemId) || client.items.getIcon?.(itemId) || getFacepunchItemIcon(shortName) || null;
    }
  }
  catch (_) { /* ignore */ }
...
function getEmbeddedItemIcon(itemId, shortName = null) {
  try {
    if (!Fs.existsSync(itemIconState.path)) {
      itemIconState.mtimeMs = 0;
      itemIconState.icons = {};
      return null;
    }

    const mtimeMs = Fs.statSync(itemIconState.path).mtimeMs;
    if (mtimeMs !== itemIconState.mtimeMs) {
      const resolvedPath = require.resolve(itemIconState.path);
      delete require.cache[resolvedPath];
      const icons = require(resolvedPath);
      itemIconState.icons = icons && typeof icons === 'object' ? (icons.icons || icons) : {};
      itemIconState.mtimeMs = mtimeMs;
    }

    const key = itemId == null ? null : itemId.toString();
    return (key && itemIconState.icons[key]) || (shortName && itemIconState.icons[shortName]) || null;
  }
  catch (_) {
    return null;
  }
...
function getFacepunchItemIcon(shortName) {
  if (!shortName || typeof shortName !== 'string') return null;
  const trimmed = shortName.trim();
  if (!trimmed || !/^[a-z0-9_.-]+$/i.test(trimmed)) return null;
  return `${FACEPUNCH_ITEM_ICON_BASE_URL}${encodeURIComponent(trimmed)}_64.png`;
}
...
function appJs() {
  ...
  function popoverOrderHtml(o){
    return '<div class="cluster-item">' + miniIcon(o.itemIcon) + '<span>' + escapeHtml((o.quantity || 0) + '× ' + o.itemName) + '</span>' + miniIcon(o.currencyIcon) + '<span>' + escapeHtml((o.cost || 0) + '× ' + o.currencyName) + '</span></div>';
  }
  ...
  function addClusterMarker(cluster){
    ...
    el.className = 'marker cluster' + (cluster.vendors.some(v => v.id === state.selectedId) ? ' selected' : ''); el.style.left = pos.x + 'px'; el.style.top = pos.y + 'px'; el.title = cluster.vendors.length + ' vending machines';
  ...
  function clusterPopoverHtml(cluster){
    return '<div class="cluster-popover"><div class="cluster-title">' + cluster.vendors.length + ' vending machines in this base</div>' + cluster.vendors.map(v => '<div class="cluster-vendor"><b>' + escapeHtml(v.grid || v.location || 'Vendor') + '</b><div class="cluster-items">' + (v.orders || []).filter(o => els.showOutOfStock.checked || o.inStock).map(popoverOrderHtml).join('') + '</div></div>').join('') + '</div>';
  }


  function vendorPopoverHtml(vendor){
    const visibleOrders = (vendor.orders || []).filter(o => els.showOutOfStock.checked || o.inStock);
    return '<div class="vendor-popover"><div class="cluster-title">' + escapeHtml(vendor.grid || vendor.location || 'Vending machine') + '</div><div class="cluster-items">' + (visibleOrders.length ? visibleOrders.map(popoverOrderHtml).join('') : '<div class="muted">No visible sell orders.</div>') + '</div></div>';
  }
  ...
  function orderHtml(o){ const left = escapeHtml((o.quantity || 0) + '× ' + o.itemName + (o.itemBlueprint ? ' BP' : '')); const right = escapeHtml((o.cost || 0) + '× ' + o.currencyName + (o.currencyBlueprint ? ' BP' : '')); return '<div class="order ' + (o.inStock ? '' : 'out') + '">' + squareIcon(o) + '<div class="item"><b>' + left + '</b><span>Stock: ' + escapeHtml(o.stock) + '</span></div><div class="arrow">for</div><div class="item"><b>' + miniIcon(o.currencyIcon) + right + '</b><span>Currency</span></div></div>'; }
